  ...
  return (
    <nav className="bg-white border-gray-200 dark:bg-gray-900">
        <div className="max-w-screen-xl flex flex-wrap lg:items-center justify-between mx-auto p-4">
            ...
            <div className='xs:hidden xxs:hidden sm:hidden md:hidden'>
                <ul className="font-medium relative flex flex-row w-fit left-2 text-black p-0 mt-0 border-0 border-gray-100 space-x-6">
                    {
                    Links.map(link => (
                    <li className='font-semibold my-5 md:my-0 md:ml-8 block pl-3 pr-4 text-black hover:text-green-700'>
        ...
        <div className='md:hidden lg:hidden xl:hidden'>    
            <ul ref={ref} className={`transition-all ${isOpen ? 'top-8' : 'top-[-290px]'} right-2  duration-500 ease-in bg-gradient-to-br from-green-200 to-emerald-400 z-[1] font-medium absolute flex flex-col p-4 w-fit md:p-0 mt-4 border border-gray-100 rounded-lg bg-green-50 md:flex-row md:space-x-6 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700`}>
                {
                  Links.map(link => (
                  <li className='font-semibold my-4 ml-2 block pl-3 pr-4 text-black hover:text-green-700'>
                      <Link href={link.link}>{link.name}</Link>
                  </li>))
                }
